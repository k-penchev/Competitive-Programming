...
string readProgram()
{
    ...
    fstream cinn;
    cinn.open("file.txt",ios_base::in);
...
double scoring(double factor)
{
    if (factor <= 13.5)
        return 1.0;
    return max(15.0 / 55.0, exp(-pow((factor / 13.5 - 1), 0.75)));
}
...
int main(int argc, char *argv[])
{
    ...
    int X, Y, seed, subtask;

    scanf("%d %d %d %d", &X, &Y, &seed, &subtask);
